      ...
      step1: [
        {
          name: 'tool_a',
          description: 'A',
          inputSchema: z.object({ name: z.string(), count: z.number().int().positive() }),
...
describe('ValidationLayer', () => {
  it('valid tool call in correct state passes', () => {
    const result = vl.validateToolCall(instanceId, 'tool_a', { name: 'foo', count: 3 })
  ...
  it('tool call with wrong type is rejected by Zod', () => {
    const result = vl.validateToolCall(instanceId, 'tool_a', { name: 'foo', count: 'bad' })
    expect(result.valid).toBe(false)
    expect(result.reason).toContain("'tool_a'")
  })
  ...
  it('all rejections appear in audit log', () => {
    ...
    const rejectedEvents = log.filter((e) => ['tool_rejected', 'validation_failed'].includes(e.eventType))
  ...
  it('validateTransitionPayload auto-discovers schema from validationsByState', () => {
    ...
    strictWm.registerDefinition({
      ...
      toolsByState: { a: [], b: [] },
      validationsByState: { a: { GO: z.object({ count: z.number().int().positive() }) } },
    })
    const strictId = strictWm.createInstance('strict').instanceId

    // Invalid payload — schema rejects it
    const bad = strictVl.validateTransitionPayload(strictId, 'GO', { count: -1 })
    expect(bad.valid).toBe(false)
    expect(bad.reason).toContain("'GO'")

    // Valid payload
    const good = strictVl.validateTransitionPayload(strictId, 'GO', { count: 3 })
  ...
  it('idempotency key detects duplicate', () => {
    ...
    const first = vl.checkIdempotency(instanceId, 'tool_a', { name: 'foo', count: 3 }, '{name}_count{count}')
    ...
    const second = vl.checkIdempotency(instanceId, 'tool_a', { name: 'foo', count: 3 }, '{name}_count{count}')
  ...
  it('idempotency key allows different inputs for same tool', () => {
    ...
    const result = vl.checkIdempotency(instanceId, 'tool_a', { name: 'foo', count: 5 }, '{name}_count{count}')
